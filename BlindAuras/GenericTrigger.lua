...
frame.unitFrames = {};
Private.frames["BlindAuras Generic Trigger Frame"] = frame;
...
do
  ...
  function BlindAuras.WatchForCastLatency()
    if not castLatencyFrame then
      ...
      -- triggered from cacheEmpoweredFrame after updating cache use by BlindAuras.UnitChannelInfo

      castLatencyFrame:SetScript("OnEvent", function(self, event)
        if event == "UNIT_SPELLCAST_START" then
          Private.LAST_CURRENT_SPELL_CAST_START = select(4, BlindAuras.UnitCastingInfo("player")) / 1000
        elseif event == "UNIT_SPELLCAST_CHANNEL_START" then
          Private.LAST_CURRENT_SPELL_CAST_START = select(4, BlindAuras.UnitChannelInfo("player")) / 1000
