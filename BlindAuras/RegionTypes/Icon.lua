...
local function AnchorSubRegion(self, subRegion, anchorType, selfPoint, anchorPoint, anchorXOffset, anchorYOffset)
  if anchorType == "area" then
    BlindAuras.regionPrototype.AnchorSubRegion(selfPoint == "region" and self or self.icon,
...
local function create(parent, data)
  ...
  --The reason is so that BlindAuras cooldown frames can interact properly with OmniCC
  ...
  then
    ElvUI[1]:RegisterCooldown(cooldown, "BlindAuras");
  end
