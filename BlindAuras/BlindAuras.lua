...
Private.frames["BlindAuras Main Frame"] = BlindAurasFrame;
...
function Private.AddMany(tbl, takeSnapshots)
  ...
  for _, data in ipairs(tbl) do
    ...
    if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame and data.anchorFrameFrame:sub(1, 10) == "BlindAuras:" then
    ...
    if idtable[target] then
      while(parent) do
        if parent == source then
          BlindAuras.prettyPrint(L["Warning: Anchoring to your own child '%s' in aura '%s' is imposssible."]:format(target, source))
...
local function pAdd(data, simpleChange)
  ...
  if simpleChange then
    db.displays[id] = data
    if BlindAuras.GetRegion(data.id) then
...
function Private.SetRegion(data, cloneId)
  ...
  if not(regionType) then
    ...
    if not(id) then
      ...
      if(data.parent) then
        if BlindAuras.GetData(data.parent) then
...
local function EnsureRegion(id)
  if not Private.regions[id] or not Private.regions[id].region then
    ...
    for _, toCreateId in ipairs_reverse(aurasToCreate) do
      local data = BlindAuras.GetData(toCreateId)
      Private.SetRegion(data)
      if (data.regionType == "dynamicgroup") then
...
function Private.EnsureRegion(id, cloneId)
  ...
  return BlindAuras.GetRegion(id)
end
  ...
  then
    ...
    if actions.glow_frame_type == "FRAMESELECTOR" then
      if actions.glow_frame:sub(1, 10) == "BlindAuras:" then
        ...
        if BlindAuras.GetData(frame_name) then
...
local function tryAnchorAgain()
  ...
  for id, _ in pairs(delayed) do
    ...
    if (data and region) then
      local parent = BlindAurasFrame;
      if (data.parent and BlindAuras.GetData(data.parent) and Private.EnsureRegion(data.parent)) then
...
local function GetAnchorFrame(data, region, parent)
  ...
  if (anchorFrameType == "SELECTFRAME" and anchorFrameFrame) then
    if(anchorFrameFrame:sub(1, 10) == "BlindAuras:") then
      ...
      local targetData = BlindAuras.GetData(frame_name)
      if targetData then
        for parentData in Private.TraverseParents(targetData) do
          if parentData.id == data.id then
            BlindAuras.prettyPrint(L["Warning: Anchoring to your own child '%s' in aura '%s' is imposssible."]:format(frame_name, data.id))
            return parent
          end
        end
      end
