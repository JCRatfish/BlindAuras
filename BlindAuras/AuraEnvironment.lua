...
function Private.ActivateAuraEnvironment(id, cloneId, state, states, onlyConfig)
  local data = id and BlindAuras.GetData(id)
  ...
  override = {
    ...
    GetData = function(id)
      ...
      if getDataCallCounts[currentId] > 99 then
        Private.AuraWarnings.UpdateWarning(current_uid, "FakeBlindAurasGetData", "warning",
                  L["This aura calls GetData a lot, which is a slow function."])
      end
      return CopyTable(BlindAuras.GetData(id))
    end,
    clones = MakeDeprecated(Private.clones, "clones",
                L["Using BlindAuras.clones is deprecated. Use BlindAuras.GetRegion(id, cloneId) instead."]),
    regions = MakeDeprecated(Private.regions, "regions",
                L["Using BlindAuras.regions is deprecated. Use BlindAuras.GetRegion(id) instead."])
  },
  blocked = blocked,
  setBlocked = function()
